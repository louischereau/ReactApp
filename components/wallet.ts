...
export async function getTransactionData(
  ...
  for (
    let dumpId = 0;
    mint != undefined || dumpId < dumpList.length;
    dumpId++
  ) {
    ...
    if (fetchedNFTMetadata) {
      ...
      nft = {
        ...
        family: fetchedNFTMetadata.collection
          ? fetchedNFTMetadata.collection.family
          : "",
      };
    }
    result.push(nft);
...
const getPriceInfo = async (
  ...
  const parsedTrxDatas = await connection.getParsedConfirmedTransaction(
    sig,
    "finalized"
  );
...
export const parseTransactionData = (
  raw: ParsedConfirmedTransaction | null,
  sig: string
) => {
    ...
    for (const ins of innerIns.instructions) {
      ...
      if (newData.accounts) {
        ...
        for (const account of newData.accounts)
          newAccounts.push(account.toBase58());
  ...
  for (const ins of newTransaction?.message.instructions) {
    ...
    if (newIns.accounts) {
      let newInsAccounts = [];
      for (const account of newIns.accounts as any)
        newInsAccounts.push(account.toBase58());
 ...
 */
export function loadDump(dumpType: string, cPath: string = DUMP_PATH) {
